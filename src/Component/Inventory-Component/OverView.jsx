...
// small count-up hook for stats (kept from your original)
function useCountUp(target, duration = 800) {
  ...
  useEffect(() => {
    ...
    function step(ts) {
      ...
      setValue(
        Math.round(from + (to - from) * (1 - Math.pow(1 - progress, 3)))
      );
...
function OverView() {
  ...
  // carousel / highlights unchanged (static for now)
  ...
  const [stockInTarget, setStockInTarget] = useState(0);
  const [stockOutTarget, setStockOutTarget] = useState(0);
  const [totalStockTarget, setTotalStockTarget] = useState(0);
  ...
  useEffect(() => {
    ...
    async function fetchStats() {
      ...
      setStatsError(null);
      try {
        // Adjust URL if your backend lives elsewhere
        const res = await axios.get("http://localhost:5000/api/products");
        const products = Array.isArray(res.data) ? res.data : [];

        // compute totals safely (handle missing or string values)
        let inSum = 0;
        let outSum = 0;
        let curSum = 0;
        products.forEach(p => {
          // If your model uses different keys, change p.stockIn/p.stockOut here
          const sin = Number(p.stockIn) || 0;
          const sout = Number(p.stockOut) || 0;
          inSum += sin;
          outSum += sout;
          curSum += sin - sout;
        });
        ...
        if (!mounted) return;
        setStockInTarget(inSum);
        setStockOutTarget(outSum);
        setTotalStockTarget(curSum >= 0 ? curSum : 0);
      } catch (err) {
        console.error("Failed to load inventory stats:", err);
        if (mounted) {
          setStatsError("Failed to load inventory overview.");
          setStockInTarget(0);
          setStockOutTarget(0);
          setTotalStockTarget(0);
        }
      } finally {
    ...
    fetchStats();
    // optional polling: uncomment to refetch periodically
    // const poll = setInterval(fetchStats, 1000 * 60); // every minute
    // return () => { mounted = false; clearInterval(poll); };
    return () => { mounted = false; };
  }, []);


  // feed totals into animated counters
  const totalStock = useCountUp(totalStockTarget);
  const stockIn = useCountUp(stockInTarget);
  const stockOut = useCountUp(stockOutTarget);
      ...
      <div className="max-w-7xl  mx-auto grid grid-cols-12 gap-6">
        ...
        <div className="col-span-12 lg:col-span-4 grid gap-6">
          ...
          <motion.div initial={{ opacity: 0, y: -6 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45 }}
            className="rounded-2xl p-6 bg-white/80 backdrop-blur-sm border border-amber-200 shadow-md">
            <div className="flex items-center gap-3 mb-3">
              ...
              <div>
                ...
                <p className="text-xs text-amber-600"> • Manager</p>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between text-amber-700 text-sm">
              ...
              <div className="flex items-center gap-2">
                <FiClock /> <span>{time.toLocaleDateString()}</span>
              </div>
            </div>
          </motion.div>

          {/* Product highlights (kept static for now) */}
          <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.06 }}
            ...
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="flex items-center gap-2">
                <button onClick={prev} aria-label="Prev" className="p-1 rounded-md hover:bg-amber-50 transition">
                  <FiArrowLeftIcon />
                </button>
                <button onClick={next} aria-label="Next" className="p-1 rounded-md hover:bg-amber-50 transition">
            ...
            <div className="relative">
              <AnimatePresence mode="wait">
                <motion.div key={productHighlights[index].id} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}
                  exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.45 }}
                  className="p-3 rounded-lg bg-amber-50 border border-amber-100 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-md bg-amber-200/40 flex items-center justify-center">
                      {productHighlights[index].icon}
                    </div>
                    <div>
              ...
              <div className="flex items-center gap-2 mt-3">
                {productHighlights.map((p, i) => (
                  <button key={p.id} onClick={() => setIndex(i)}
                    className={`w-2 h-2 rounded-full transition-transform ${i === index ? "scale-125 bg-amber-500" : "bg-amber-200"}`}></button>
                ))}
              </div>
            </div>
          </motion.div>
        ...
        <div className="col-span-12 lg:col-span-8 grid grid-rows-12 gap-6">
          ...
          <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} className="row-span-4 my-[40px] grid grid-cols-12 gap-3">
            <div className="col-span-9 grid grid-cols-3 gap-4">
              <motion.button whileHover={{ scale: 1.03 }}  className="rounded-xl p-5 bg-gradient-to-r from-amber-600 to-amber-700 text-white shadow-lg flex flex-col items-center justify-center">
                <a href="/cofinvoicedashboard/create-invoice">
              ...
              <motion.button whileHover={{ scale: 1.03 }} className="rounded-xl p-5 bg-white border border-amber-200 shadow-md flex flex-col items-center justify-center">
                <a href="/cofinvoicedashboard/invoices">
              ...
              <motion.button whileHover={{ scale: 1.03 }} className="rounded-xl p-5 bg-[#635c5c54] border border-amber-200 shadow-md flex flex-col items-center justify-center">
               <a href="/cofdashboard/new-records">
          ...
          <motion.div initial="hidden" animate="show" variants={{ show: { transition: { staggerChildren: 0.05 } } }}
            className="row-span-8 bg-transparent backdrop-blur-sm rounded-2xl p-6 border border-amber-200 shadow-md">
            <h3 className="text-lg font-semibold text-amber-900 mb-6">Inventory Overview</h3>

            {/* Loading/Error */}
            {loadingStats ? (
              <div className="py-8 text-amber-700">Loading inventory stats…</div>
            ) : statsError ? (
              <div className="py-8 text-red-600">{statsError}</div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <motion.div variants={card} className="rounded-xl p-5 bg-gradient-to-br from-amber-500 to-amber-600 text-white shadow-lg">
                  <div className="flex justify-between items-start mb-4">
                    ...
                    <div className="bg-amber-400/20 p-1 rounded"><FiBox className="text-white" /></div>
                  </div>
