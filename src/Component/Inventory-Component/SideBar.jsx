  ...
  return (
    <>
      ...
      <button
        ...
        className="lg:hidden fixed top-4 left-4 z-50 p-2 rounded-md bg-gradient-to-r from-yellow-600 to-yellow-700 text-white shadow-md"
      ...
      >
        {/* decorative stripe */}
        <div className="absolute left-0 top-0 h-full w-1 bg-gradient-to-b from-yellow-600 to-yellow-700/90 pointer-events-none" />

        {/* header */}
        <div className="flex items-center gap-3 p-4 border-b border-amber-100">
          <div className="w-10 h-10 rounded-md bg-gradient-to-br from-yellow-600 to-yellow-700 flex items-center justify-center text-white font-bold shadow-sm">COF</div>
          <AnimatePresence>
            {isOpen && (
        ...
        <div className="p-4 flex items-center gap-3 border-b border-amber-100">
          <div className="w-12 h-12 rounded-full bg-gradient-to-br from-yellow-600 to-yellow-700 flex items-center justify-center text-white font-semibold shadow">A</div>
          <AnimatePresence>
            {isOpen && (
                  ...
                  <motion.div initial={{ backgroundColor: 'transparent' }} whileHover={{ backgroundColor: 'rgba(183,123,29,0.06)' }} transition={{ duration: 0.12 }} className="rounded-md">
                    <Link
                      ...
                      className={`group flex items-center gap-3 w-full px-3 py-2 rounded-md transition-all focus:outline-none ${
                        isActive ? 'bg-gradient-to-r from-yellow-600 to-yellow-700 text-white shadow' : 'text-amber-900'
                      }`}
                    >
                      <motion.span
                      ...
                      {isActive && <motion.div className="ml-auto w-1 h-6 rounded-full bg-gradient-to-b from-yellow-400 to-yellow-600" initial={{ opacity: 0, scaleX: 0 }} animate={{ opacity: 1, scaleX: 1 }} transition={{ duration: 0.14 }} />}
                    </Link>
                  </motion.div>
                </li>
              )
